    ...
    <>
      <motion.nav 
        ...
        className={`fixed top-0 left-0 right-0 z-50 border-b px-6 md:px-8 py-4 md:py-5 flex items-center justify-between shadow-sm transition-colors duration-700 ${
          isTechTheme 
            ? 'bg-zinc-950/90 backdrop-blur-md border-amber-500/20 text-zinc-100' 
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 md:w-10 md:h-10 border rounded-full flex items-center justify-center font-serif text-lg md:text-xl transition-colors duration-700 ${
            isTechTheme ? 'border-amber-500 text-amber-500' : 'border-bone-ink text-bone-ink'
          }`}>
            甲
          </div>
          ...
          {links.map((link) => (
            <NavLink
              ...
              className={({ isActive }) => `
                ...
                ${isActive 
                  ? (isTechTheme ? 'text-amber-500 font-bold' : 'text-bone-brown font-bold') 
                  : (isTechTheme ? 'text-zinc-400 hover:text-amber-500' : 'text-bone-ink/60 hover:text-bone-brown')
                }
                ${link.isTech && !isTechTheme && !isActive ? 'hover:text-amber-500' : ''}
              `}
            >
              {({ isActive }) => (
                <>
                  ...
                  {isActive && (
                    <motion.div
                      ...
                      className={`absolute bottom-0 left-0 right-0 h-0.5 ${isTechTheme ? 'bg-amber-500' : 'bg-bone-brown'}`}
                    />
                  )}
                </>
              )}
              ...
              {links.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) => `
                    border-b pb-4
                    ${isActive 
                      ? (isTechTheme ? 'text-amber-500 border-amber-500/30' : 'text-bone-brown border-bone-brown/30') 
